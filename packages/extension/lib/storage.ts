...
/**
 * The Simkl stops that are waiting out the 20 s lock (request body and when the
 * wait ends), keyed by a per-stop id. Several can wait at once (two tabs stop
 * inside one lock). The worker normally sends each after its wait. If the browser
 * stops the worker during the wait, an alarm sends them instead, so no watch is
 * lost (constraint #4: no timer kept in memory is the only record of it).
 */
export const simklHeldStops = storage.defineItem<
  Record<string, { due: number; body: Record<string, unknown> }>
>("local:simkl_held_stops", { fallback: {} });
