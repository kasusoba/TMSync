...
/** One frame from `webNavigation.getAllFrames` — the ground-truth committed URL. */
...
/** Per-frame video data from the executeScript pass, keyed by frameId. */
export interface FrameVideoReport {
  frameId: number;
  title: string;
  videos: VideoInfo[];
}
...
/**
 * Build the frame tree from `webNavigation.getAllFrames` (exact parentage + each
 * frame's REAL committed URL, even for frames we can't inject into) enriched with
 * video data from the executeScript pass. This is the accurate path: it sees
 * through the iframe-`src` lie that redirect-chain embeds (vsrc.su → cloudnestra)
 * play on the DOM, so the origin shown is the one actually worth enabling.
 *
 * Non-http frames (about:blank ad slots, sandboxed srcdoc) are dropped and their
 * http descendants re-parented to the nearest kept ancestor.
 */
export function buildFrameTreeFromNav(
  nav: NavFrame[],
  reports: FrameVideoReport[],
  enabledOrigins: string[],
): FrameNode[] {
  const enabled = new Set(enabledOrigins);
  const reportByFrame = new Map(reports.map((r) => [r.frameId, r]));
  const navById = new Map(nav.map((f) => [f.frameId, f]));
  const keep = nav.filter((f) => f.frameId === 0 || isHttp(f.url));

  const nodeById = new Map<number, FrameNode>();
  for (const f of keep) {
    const r = reportByFrame.get(f.frameId);
    const reals = realVideos(r?.videos ?? []);
    nodeById.set(f.frameId, {
      frameId: f.frameId,
      url: f.url,
      origin: originOf(f.url),
      isTop: f.frameId === 0,
      reached: r !== undefined,
      enabled: enabled.has(originOf(f.url)),
      title: r?.title ?? "",
      videos: r?.videos ?? [],
      hasVideo: reals.length > 0,
      hasActiveVideo: reals.some((v) => !v.paused && v.readyState >= 2),
  ...
  // Nearest ancestor that survived the http filter (skips dropped ad/srcdoc frames).
  const keptParentId = (f: NavFrame): number | null => {
    let pid = f.parentFrameId;
    while (pid !== -1) {
      if (nodeById.has(pid)) return pid;
      const parent = navById.get(pid);
      if (!parent) break;
      pid = parent.parentFrameId;
    }
    return null;
  };
  ...
  for (const f of keep) {
    const node = nodeById.get(f.frameId);
    if (!node) continue;
    const pid = f.frameId === 0 ? null : keptParentId(f);
    const parent = pid !== null ? nodeById.get(pid) : undefined;
