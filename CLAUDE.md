...
2. **Anime is AniList-only, dedicated-anime-sites-only.** Anime *series* scrobble to AniList; anime *movies* still go to Trakt. **Anime is supported only on dedicated anime sites** — where a page already corresponds to one AniList-ish entry and numbering lines up, so no offset arithmetic is needed. The general-site anime path is a **non-goal (decided 2026-06), not merely deferred**: do NOT build absolute↔AniList-entry offset mapping, the TMDB/general-site anime path, or an is-anime classifier. Rationale: anime viewers use dedicated anime sites; watching anime on a general TMDB site would only serve people wanting Trakt↔AniList parity, which TMSync explicitly is not (constraint #1). If this is ever revisited, it must be quarantined inside the AniList adapter and never leak into the shared `extract()` engine.
...
- Do not build the anime hard-cases (absolute↔entry offset mapping, TMDB/general-site anime path, is-anime classifier) — **non-goal, not just deferred**. Anime is dedicated-anime-sites-only; the general-site path only serves Trakt↔AniList parity, which TMSync isn't.
