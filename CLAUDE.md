...
1. **Exactly two trackers — anime may be multi-tracked to both.** Trakt and AniList only; **no third** (no Simkl/MAL/Kitsu/Letterboxd). **Anime (movies + series) may be written to BOTH** trackers at once (multi-track — `docs/MULTI-TRACK.md`); everything non-anime is **Trakt-only by nature** (it doesn't exist on AniList). The old "one item → one tracker, never synced" rule is **retired** (2026-07). Each tracker is one implementation behind the adapter seam (see **Tracker adapters**); a watch session fans out to the resolved trackers. Multi-track = **anime only** — the sole content type on both AniList and Trakt.
...
- Do not add a **third** tracker (Simkl, MAL, Kitsu, TMDB-as-tracker, …). Trakt + AniList only. (Letterboxd is a CSV *export target*, not a tracker — that's fine.)
