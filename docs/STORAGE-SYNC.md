...
2. **Migrate user-authored config to `browser.storage.sync`** — DONE (2026-06-29): flipped `custom_recipes`, `quick_links`, `corrections`, `manual_selections` to `sync:` single keys (no migration shim — accepted wipe at this stage). Sync set: **custom recipes, quicklinks, corrections, manual picks** (not rating/note mirrors). Follow-ups: per-item keys for quota safety. DONE for custom recipes (2026-09-19): one `recipe:{id}` key each, read through the schema, the old `custom_recipes` key is moved on the first write. Quick links, corrections, and manual picks are still single keys. The "set up these sites here" re-grant prompt shipped in v2.6.0.
