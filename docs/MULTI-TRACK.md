...
> **Update 2026-09:** "everything else stays Trakt-only" is relaxed. Non-anime may also go to Simkl, which takes the page's own numbering (the `any` family), so no crosswalk is involved. See `TRACKERS-PLAN.md`.
...
- **No `SCHEMA_VERSION` bump.** The field is additive-optional, so v1/older-v2 recipes are unchanged, and an older engine that ignores `trackers` degrades to native-only (`[tracker]`): a graceful degrade, *better* than a version gate that would make old clients drop a multi-track recipe entirely. (Adding MAL and Simkl as tracker VALUES did need one: an older build cannot parse `"mal"` or `"simkl"`, so a recipe that names one carries `schemaVersion: 4`. Trakt and AniList recipes stay at 3.)
